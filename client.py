...
if __name__ == '__main__':
    ...
    print("[Checkpoint] Connected to vhost '{0}' on RMQ server at '{1}' as user '{2}'".format(rmq_params.get("vhost"),
                                                                                              RMQ_IP, rmq_params.get("username")))

    try:
        channel.queue_bind(exchange=rmq_params.get("exchange"), queue=rmq_params.get("client_queue"),
                           routing_key=rmq_params.get("client_queue"))
        channel.basic_consume(callback, queue=rmq_params.get("client_queue"), no_ack=True)
        print("[Checkpoint] Consuming from RMQ queue: {0}".format(rmq_params.get("client_queue")))
        channel.start_consuming()
    except:
        print("[ERROR] The queue ({0}) was not found or the client.py process was killed".format(
            rmq_params.get("client_queue")))
